...
function dbIsReady() {
  docker-compose logs users-db | grep "PostgreSQL init process complete"
}
function flaskIsReady() {
  docker-compose logs users | grep "Serving Flask app"
}
...
waitUntilServiceIsReady flaskIsReady "Flask"
